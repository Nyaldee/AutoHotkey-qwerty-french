...
Esc::Send "#{d}"                           ; F15 + Échap → Minimise / Restaure toutes les fenêtres
...
CenterWindow80() {
    ...
    DllCall("SetWindowPos", "ptr", hwnd, "ptr", 0, "int", newX, "int", newY, "int", newW, "int", newH, "uint", flags, "void")
}

; -----------------------------------------------------------------------------
; Fonction utilitaire : Disposer toutes les fenêtres visibles en cascade centrée
; -----------------------------------------------------------------------------
CascadeWindowsCentered() {
    ...
    for hwnd in WinGetList() {
        try {
            ...
            if !IsSet(title) || title == ""                 ; Sans titre (peut être unset en mode v2.1)
                continue
